package com.elektro.monitoring.helper

object Constants {
    const val SPLASH_DELAY = 2500
    const val SHARED_PREFERENCES_FILE_NAME = "monitoring"
    const val ACTION_START_TEMPERATURE_MONITORING = "com.elektro.monitoring.action.START_TEMPERATURE_MONITORING"
    const val PASSWORD_PATTERN = "^(?=.*[0-9])(?=.*[A-Z])(?=.*[@#$%^&+=!-])(?=\\S+$).{8,}$"
    const val NOTIFICATION_CHANNEL_ID = "your_channel_id"
    const val NOTIFICATION_ID = 1
    const val NOTIFICATION_MESSAGE_TAG = "message from notification"
    const val TAG = "NotificationRepository"
}